...
def _get_scenario(A, u_xi, q_in):
    """Cached loader.load_scenario for one True-file scenario (fields+ctrl)."""
    key = (A, u_xi, q_in)
    if key not in _SC_CACHE:
        sys.path.insert(0, str(BASE))
        import loader
        path = DATA_DIR / f"data_{A:g}_{u_xi:g}_{q_in:g}_True.mat"
        _SC_CACHE[key] = loader.load_scenario(path, fields=True, ctrl=True)
    return _SC_CACHE[key]


def _load_field(A, u_xi, q_in, rep):
    """(rho[t,x] veh/km, t[s], x[km]) for a True-file scenario rep."""
    sc = _get_scenario(A, u_xi, q_in)
    rho = np.asarray(sc.reps[rep].rho, float)
    return rho, sc.t_field, sc.x_cells / 1000.0


def _load_ctrl_xcav(A, u_xi, q_in, rep, t_grid):
    """CAV position [km] on t_grid from the controlled-vehicle trajectory."""
    sc = _get_scenario(A, u_xi, q_in)
    c = sc.reps[rep].ctrl
    return np.interp(t_grid, c.t, c.x, left=np.nan, right=np.nan) / 1000.0


def _queue_tail_km(A, u_xi, q_in, rep, t_grid):
    """Queue-tail position [km] on t_grid.

    A in {1, 10}: x_cav - L_zone from out/e1/A{A}_u{u}_q{q}_r{rep}.npz.
    A = 3 (no npz): x_cav(ctrl) - 2.5 km conservative proxy.
    """
    if A in (1, 10):
        p = E1_DIR / f"A{A:g}_u{u_xi:g}_q{q_in:g}_r{rep}.npz"
        if p.exists():
            with np.load(p) as z:
                tail = (np.asarray(z["x_cav"], float)
                        - np.asarray(z["L_zone"], float)) / 1000.0
                tt = np.asarray(z["tt"], float)
                ok = np.isfinite(tail)
                return np.interp(t_grid, tt[ok], tail[ok])
